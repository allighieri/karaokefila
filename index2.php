...
<head>
    ...
    <style>
        /* Estilos para o corpo e HTML para remover rolagem vertical padrão */
        body, html {
            overflow: hidden; /* O carrossel do Bootstrap gerencia isso */
            margin: 0;
            padding: 0;
            width: 100%;
            height: 100%;
        ...
        .carousel-inner,
        .carousel-item {
            height: 100vh; /* Faz com que o carrossel ocupe a altura total da viewport */
            width: 100vw; /* Garante que cada item ocupe a largura total */
            overflow: hidden; /* Esconde conteúdo extra se houver problemas de layout */
        }

        /* Estilo de cada seção (agora dentro de .carousel-item) */
        .carousel-item .content-wrapper {
            max-width: calc(100% - 20px); /* Ocupa quase 100% da largura, deixando 20px no total para margens/padding */
            width: 100%;
            padding: 80px 10px 20px 20px; /* Reduzimos o padding-right para 10px */
            box-sizing: border-box;
            margin: 0 auto;
            font-size: 2em; /* Mantendo o tamanho da fonte */
            height: 100%; /* Ocupa a altura total do item do carrossel */
            overflow-y: auto; /* Permite rolagem vertical dentro da seção se o conteúdo for grande */
            -webkit-overflow-scrolling: touch;
        }

        /* Cores de exemplo para as seções/itens do carrossel */
        #sectionOne,#sectionTwo, #sectionThree, #sectionFour { background-color: #d2fce9; }


        /* Estilos para os botões de navegação lateral do carrossel */
        .carousel-control-prev, .carousel-control-next {
            z-index: 1050; /* Garante que fiquem acima do conteúdo */
            width: 5%; /* Ajuste a largura da área clicável se quiser */
        ...
        /* Cor dos ícones dos controles */
        .carousel-control-prev-icon, .carousel-control-next-icon {
            filter: invert(1); /* Torna os ícones brancos se o fundo for escuro */
        }

        /* Estilos para o navbar fixo no topo */
        .navbar {
            z-index: 1060;
        ...
        /* Estilo específico para o conteúdo do parágrafo na seção 1 */
        .carousel-item p { /* Aplicado a todos os parágrafos dentro de carousel-item */
            font-size: 0.6em;
            text-align: justify;
            margin-bottom: 1em;
            max-width: 800px;
        }
        /* Responsividade para o conteúdo da seção para telas menores */
        @media (max-width: 768px) {
            .carousel-item .content-wrapper {
                font-size: 1.5em;
                padding-top: 60px;
                padding-left: 10px;
                padding-right: 10px; /* Mantém o padding direito menor para mobile também */
            }
            .carousel-item p {
                font-size: 0.7em;
            }
...
<nav class="navbar navbar-expand-lg navbar-dark fixed-top">
    <div class="container-fluid">
        <a class="navbar-brand" href="#">Gerenciador de Filas Karaokê</a>
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto">
                <li class="nav-item">
                    <a class="nav-link active" aria-current="page" data-bs-slide-to="0" href="#sectionOne">Gerenciar Fila</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" data-bs-slide-to="1" href="#sectionTwo">Montar Rodada</a>
                </li>
                <li class="nav-item">
...
<div id="mainCarousel" class="carousel slide" data-bs-interval="false">
    <div class="carousel-inner">
        <div class="carousel-item active" id="sectionOne">
        ...
        <div class="carousel-item" id="sectionTwo">
            <div class="content-wrapper">
        ...
        <div class="carousel-item" id="sectionThree">
            <div class="content-wrapper">
                <div class="container">
        ...
        <div class="carousel-item" id="sectionFour">
            <div class="content-wrapper">
                <div class="container">
                    <div class="form-section my-5">
...
<script>
    $(document).ready(function() {
        ...
        const carouselItems = $mainCarousel.find('.carousel-item');

        // Referência direta ao elemento DOM do carrossel
        const myCarouselElement = document.getElementById('mainCarousel');
        let myCarousel; // Declara a variável aqui para que seja acessível em todo o escopo do ready

        // Função para rolar o conteúdo de um slide para o topo suavemente
        function scrollToTopSmoothly($element) {
            $element.stop().animate({
                scrollTop: 0
            }, 400); // 400ms de duração, ajuste conforme preferir
        }

        // --- LÓGICA DE PERSISTÊNCIA DO SLIDE ATUAL E INICIALIZAÇÃO ---

        const storedSlideIndex = localStorage.getItem('currentSlideIndex');
        const initialHash = window.location.hash;

        let initialSlide = 0; // Padrão: primeiro slide

        if (initialHash) {
            const indexFromHash = $navLinks.filter(`[href="${initialHash}"]`).data('bs-slide-to');
            if (typeof indexFromHash !== 'undefined') {
                initialSlide = indexFromHash;
            }
        } else if (storedSlideIndex !== null) {
            initialSlide = parseInt(storedSlideIndex, 10);
        }

        // Inicializa o carrossel. A classe 'slide' já está no HTML, então a transição será padrão.
        myCarousel = new bootstrap.Carousel(myCarouselElement, {
            interval: false,
            wrap: false
        });

        // Usa setTimeout para garantir que o Bootstrap já inicializou completamente
        setTimeout(() => {
            // Ativa o carrossel no slide inicial determinado. Isso agora incluirá a transição.
            myCarousel.to(initialSlide);

            // Também garante que a rolagem esteja no topo ao carregar a página
            const $initialContentWrapper = $(carouselItems[initialSlide]).find('.content-wrapper');
            scrollToTopSmoothly($initialContentWrapper);

            // Atualiza a classe 'active' do nav-link correspondente ao slide inicial
            $navLinks.removeClass('active');
            $navLinks.filter(`[data-bs-slide-to="${initialSlide}"]`).addClass('active');

        }, 50); // Pequeno atraso para garantir que o carrossel esteja pronto

        // --- FIM DA LÓGICA DE PERSISTÊNCIA E INICIALIZAÇÃO ---

        // Evento para cliques nos links do menu
        $navLinks.on('click', function(e) {
            e.preventDefault();
            const slideToIndex = $(this).data('bs-slide-to');

            if (typeof slideToIndex !== 'undefined') {
                // Chama o método 'to' do carrossel para mudar o slide
                myCarousel.to(slideToIndex);
                // Fecha o menu hamburguer se estiver aberto (para mobile)
                $('.navbar-collapse').collapse('hide');
            }
        });

        // Evento do carrossel para atualizar a classe 'active' do menu,
        // rolar o conteúdo para o topo, E SALVAR O SLIDE ATUAL NO LOCALSTORAGE
        $mainCarousel.on('slid.bs.carousel', function () {
            // Rolar o conteúdo do slide ATIVO para o topo suavemente
            const $activeContentWrapper = $mainCarousel.find('.carousel-item.active .content-wrapper');
            scrollToTopSmoothly($activeContentWrapper);

            const currentItem = $mainCarousel.find('.carousel-item.active');
            const currentIndex = carouselItems.index(currentItem);

            // Salvar o índice do slide atual no localStorage
            localStorage.setItem('currentSlideIndex', currentIndex);

            // Atualiza a classe 'active' do nav-link correspondente
            $navLinks.removeClass('active');
            $navLinks.filter(`[data-bs-slide-to="${currentIndex}"]`).addClass('active');
        });

        // Evento para o botão "Começar" na Section 1
        $('#sectionOne .btn').on('click', function(e) {
            e.preventDefault();
            const slideToIndex = $(this).data('bs-slide-to');
            if (typeof slideToIndex !== 'undefined') {
                myCarousel.to(slideToIndex);
