...
function renderizarTabelaUsuarios(containerId, usuarios, isAtivo) {
    ...
    usuarios.forEach(usuario => {
        ...
        tableHtml += `
            <tr>
                <td>
                    <button class="btn btn-link p-0 text-start text-decoration-none" onclick="visualizarUsuario(${usuario.id})" title="Visualizar Detalhes">
                        ${escapeHtml(usuario.nome)}
                    </button>
                </td>
                <td>${escapeHtml(usuario.tenant_nome || 'N/A')}</td>
                <td><span class="badge-nivel-clickable" onclick="alterarNivel(${usuario.id}, '${escapeHtml(usuario.nome)}', '${usuario.nivel}')" style="cursor: pointer;" title="Clique para alterar nível">${nivelBadge}</span></td>
                <td>
...
function getAcoesUsuarioAtivo(usuario) {
    return `
        <div class="d-flex flex-nowrap gap-1">
            <button class="btn btn-sm btn-primary" onclick="visualizarUsuario(${usuario.id})" title="Visualizar Detalhes">
                <i class="bi bi-eye"></i>
            </button>
            <button class="btn btn-sm btn-warning" onclick="editarUsuario(${usuario.id})" title="Editar Usuário">
                <i class="bi bi-pencil-square"></i>
            </button>
            <button class="btn btn-sm btn-secondary" onclick="desativarUsuario(${usuario.id}, '${escapeHtml(usuario.nome)}')" title="Desativar Usuário">
                <i class="bi bi-box-arrow-down"></i>
            </button>
            <button class="btn btn-sm btn-danger" onclick="excluirUsuario(${usuario.id}, '${escapeHtml(usuario.nome)}')" title="Excluir Permanentemente">
...
function getAcoesUsuarioInativo(usuario) {
    return `
        <div class="d-flex flex-nowrap gap-1">
            ...
            <button class="btn btn-sm btn-success" onclick="reativarUsuario(${usuario.id}, '${escapeHtml(usuario.nome)}')" title="Reativar Usuário">
    ...
    fetch(`api_usuarios.php?acao=obter_usuario&id=${id}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('visualizar-email').textContent = usuario.email || 'N/A';
